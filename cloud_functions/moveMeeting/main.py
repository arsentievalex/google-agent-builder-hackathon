...
@app.route('/moveMeeting', methods=['POST'])
def move_meeting(request):
    ...
    try:
        ...
        # Calculate the new start and end times by adding 15 minutes
        start_time = datetime.fromisoformat(event['start']['dateTime'])
        end_time = datetime.fromisoformat(event['end']['dateTime'])

        new_start = (start_time + timedelta(minutes=15)).isoformat()
        new_end = (end_time + timedelta(minutes=15)).isoformat()
        ...
        response = {
            "message": "Event moved and email sent successfully",
        }
